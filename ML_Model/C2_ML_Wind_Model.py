"""
Random Forest ML Model für die Winderzeugung in Baden-Württemberg
"""
...
def wind_model():
    ...
    X_wind_full = X_wind_full[(X_wind_full["STG_weather.main"].isin(weather_main)) &
                              (X_wind_full["FRB_weather.main"].isin(weather_main)) &
                              (X_wind_full["MAN_weather.main"].isin(weather_main)) &
                              (X_wind_full["RAV_weather.main"].isin(weather_main))]

    """
    02 DATA PREPARATION
    """
    # Separate target from predictors
    y_wind = X_wind_full["Wind_Onshore[MWh]"]
    X_wind_full.drop(["Wind_Onshore[MWh]"], axis=1, inplace=True)
    ...
    X_train, X_valid, y_train, y_valid = train_test_split(X_wind_full, y_wind, train_size=0.8, test_size=0.2, random_state=1)
    ...
    Wind_Vergleich = pd.DataFrame()
    Wind_Vergleich["Base_Wind[MWh]"] = y_valid.reset_index(drop=True)
    Wind_Vergleich["Predict_Wind[MWh]"] = preds
    Wind_Vergleich["Diff"] = Wind_Vergleich["Base_Wind[MWh]"] - Wind_Vergleich["Predict_Wind[MWh]"]

    # Export model
    dump(clf, '../data/wind_model.joblib')
